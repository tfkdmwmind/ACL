...
def generate_attention_map( model, args, epoch ):
    ...
    for i, (images, image_ids, _ ) in enumerate( eval_data_loader ):
        images = to_var( images )
        generated_captions,alpha=model.sampler(images)
        if torch.cuda.is_available():
        ...
        # Build caption based on Vocabulary and the '<end>' token
        for image_idx in range( alpha.shape[0] ):
            results[int( image_ids[ image_idx ] )]=alpha[image_idx]
